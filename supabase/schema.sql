...
-- ------------------------------------------------------ mentorship_proposals
-- A coordinator proposes a mentor for a request; the mentorship is created only
-- when the mentor accepts (see accept_proposal / decline_proposal).
...
declare v_mentor uuid; v_mentee uuid; v_req uuid; v_mid uuid; v_active int;
begin
  select mentor_id, mentee_id, request_id into v_mentor, v_mentee, v_req
...
begin
  ...
  if v_req is not null then update public.mentorship_requests set status = 'new' where id = v_req; end if;
end; $$;
...
-- ------------------------------------------------------ admin_notifications
-- Queue of routine coordinator notifications, rolled up into a periodic digest
-- email by /api/cron/admin-digest. Service-role only (RLS on, no policies).
...
create index if not exists admin_notifications_unsent_idx on public.admin_notifications (sent_at, created_at);
